...
model <- lm(USRevenue ~ Ratings + Theatres + Opening + Budget)
summary(model)


model_budget <- lm(USRevenue ~ Budget)
model_budget_opening <- lm(USRevenue ~ Opening + Budget)
anova(model_budget, model_budget_opening)


anova(model_budget, model)


help("predict()")

newdata <- data.frame(Budget = 30, Opening = 19.83, Theatres = 3065, Ratings= 7.6)
pi<-predict.lm(model,newdata,interval = c("prediction"), level = 0.99); pi
