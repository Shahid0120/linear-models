...
#a
untransformed_model <- lm(mpg ~ disp, data=cars)
transformed_model <- lm(log.mpg ~ log.disp, data=cars)

cars$dummy = c(rep(0,24),1,rep(0,7))

cars

refited_model <- lm(log.mpg ~ log.disp + dummy, data=cars)

summary(refited_model)

# c
rstudent(transformed_model)
pt(0.98,29, lower.tail = FALSE)
